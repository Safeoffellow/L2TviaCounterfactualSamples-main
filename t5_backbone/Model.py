...
from mindnlp.models.t5.t5 import T5Stack, T5ForConditionalGeneration


class EncoderAttnMaskT5Stack(T5Stack):
    def construct(
        self,
        input_ids=None,
        attention_mask=None,
        ...
        if inputs_embeds is None:
            ...
            inputs_embeds = self.embed_tokens(input_ids.astype(ms.int64))
        ...
        # NOTE in custom model, we feed attention mask like [bs, seq_len, seq_len], this only works in encode proceed
        if self.is_decoder and encoder_attention_mask is not None and encoder_attention_mask.dim() == 3:
            encoder_attention_mask = None
        ...
        if self.is_decoder and encoder_attention_mask is None and encoder_hidden_states is not None:
            ...
            encoder_attention_mask = ops.ones(
                (batch_size, encoder_seq_length),ms.int64
            )
...
class AMT5ForConditionalGeneration(T5ForConditionalGeneration):
    def __init__(self, config):
        ...
        self.encoder = EncoderAttnMaskT5Stack(encoder_config, self.shared)

        decoder_config = copy.deepcopy(config)
        decoder_config.is_decoder = True
        decoder_config.is_encoder_decoder = False
        decoder_config.num_layers = config.num_decoder_layers
        self.decoder = EncoderAttnMaskT5Stack(decoder_config, self.shared)
...
class SequencePipeline(object):
    ...
    def __init__(self, ptm_path: str, beam_size, empty_token, stop_token, max_length=50):
        self.model: AMT5ForConditionalGeneration = AMT5ForConditionalGeneration.from_pretrained(ptm_path)
    ...
    def extract_input(self, batch_dict):
        # build batch encoder input ids
        enc_in = ms.Tensor(batch_dict['enc_in'])
        context = ms.Tensor(batch_dict['context'])
        x = ms.cat((enc_in, context), -1)
        
        batch_size = batch_dict['dec_out'].shape[0]

        # extract input tensors
        max_dec_len = batch_dict['dec_out'].shape[1]
        decoder_len = batch_dict['dec_len']
        decoder_input = batch_dict['dec_in']
        decoder_output = batch_dict['dec_out']

        return x, batch_size, max_dec_len, decoder_len, decoder_input, decoder_output
    ...
    def decode_batch(self, batch_dict, prefix_mode=''):
        ...
        x = batch_dict[f'{prefix_mode}enc_in']
        enc_len = x.shape[1]
        ...
        generated = self.model.generate(
            ...
            attention_mask=batch_dict[f'{prefix_mode}attention_mask']
            # table_embed=table_tensor
        )
        return generated
