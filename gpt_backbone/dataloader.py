...
class OrgDataMng(object):
    ...
    def __init__(self, data_path) -> None:
        ...
        self.train = Tensor(json.load(
            open(os.path.join(data_path, 'train.json'), 'r')))
        self.valid = Tensor(json.load(
            open(os.path.join(data_path, 'valid.json'), 'r')))
        self.test = Tensor(json.load(
            open(os.path.join(data_path, 'test.json'), 'r')))
...
class DataLoader:
    ...
    def get_batch(self):
        ...
        for key in batch_data:
            batch_data[key] = Tensor(batch_data[key])
            batch_data[key] = ops.Cast(batch_data[key], ms.int64)
        return batch_data
