...
def evaluate(enc, pipeline: SequencePipeline, output_path, save_ckpt, valid_iterator, best_score, step_id, epoch, data_path, mode='valid'):
    pipeline.model.eval()
    ...
    if prefix_mode != '':
        ...
        if best_score < acc:
            ...
            if save_ckpt is not None:
                ...
                ms.save_checkpoint(pipeline.model, save_f)
        return best_score
...
def main():
    ...
    if args.mode == 'resume' or args.mode == 'test':
        ...
        ms.load_param_into_net(pipeline.model,ms.load_checkpoint(args.load_ckpt))
    ...
    if args.mode in ('train', 'resume'):
        ...
        optimizer = nn.SGD(pipeline.model.parameters(), args.lr)
        if args.fd:
            fd_optimizer = nn.SGD(pipeline.model.parameters(), args.lr * 0.5)
        ...
        net_with_criterion = nn.WithLossCell(pipeline.model, pipeline.loss)
        ...
        for epoch in range(args.epochs):
            ...
            for batch_dict in train_t:
                ...
                if (step_id + 1) % display_batch == 0:
                    ...
                    tmp_score = evaluate(enc, pipeline, output_path, save_ckpt, valid_iterator,
                                          best_score, step_id, epoch, args.data_path)
                    if args.inspection:
                        tmp_cd_score = evaluate(enc, pipeline, output_path, None, cd_iterator, best_cd_score, step_id, epoch, 'CD/data/', mode='test')
            ...
            if args.fd:
                ...
                net_with_criterion = nn.WithLossCell(pipeline.model, pipeline.loss)
                train_network = nn.TrainOneStepCell(net_with_criterion, fd_optimizer)
                train_network.set_train()
